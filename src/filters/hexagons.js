...
export default class HexagonFilter extends Filtro {
    ...
    makeControls() {
        const pasta = this.gui.addFolder("Hexagons")
        pasta.add(this.parameters, "Remove Filter")
        pasta.add(this.shaderPass.uniforms.filterOn, "value").name("Filter On");
        pasta.add(this.shaderPass.uniforms.tilings, "value",10,50).name("Hexagons");
        pasta.add(this.shaderPass.uniforms.intensidade, "value", {"Average":0, "Luminance":1, "Maximum":2, "Minimum":3}).name("Intensity Calculation");
        return [pasta]
    }
