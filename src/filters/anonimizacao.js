...
export default class AnonimizacaoFilter extends Filtro {
    ...
    makeControls() {
        ...
        const filtroOnController = this.gui.add(this.shaderPass.uniforms.filterOn, "value").name("Filter On")
        const faceOnlyController = this.gui.add(this.shaderPass.uniforms.faceOnly, "value").name("Face Only")
        ...
        this.parentManager.addTab(desfoque, [
            this.gui.add(this.shaderPass.uniforms.blurRadius, "value",10,40).name("Radius").hide()
        ])
        this.parentManager.addTab(glitch, [
            this.gui.add(this.shaderPass.uniforms.instabilidade, "value",0,1).name("Instability").hide(),
            this.gui.add(this.shaderPass.uniforms.eyeBlock, "value").name("Stripes").hide()
        ])
        this.parentManager.addTab(pixel, [
            this.gui.add(this.shaderPass.uniforms.pixelate, "value",0,1).name("Pixelation")
        ])
        return []
    }
